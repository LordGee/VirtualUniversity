    ...
    void Update() {
        ...
        if (player.target != null && player.target is Npc &&
            ...
            if (itemIndex != -1 && itemIndex < player.inventory.Count &&
                ...
                player.inventory[itemIndex].item.template is PetItemTemplate) {
                ...
                if (template.petPrefab != null) {
                    ...
                    costsText.text = template.petPrefab.revivePrice.ToString();
                    reviveButton.interactable = slot.item.petHealth == 0 && player.gold >= template.petPrefab.revivePrice;
